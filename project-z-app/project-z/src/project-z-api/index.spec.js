...
describe('project z api', () => {
  ...
  describe('register user', () => {
    ...
    it('should fail on empty password confirmation', () => {
      ...
      }).toThrow(new EmptyError('passwordConfirmation is empty or blank'))
    })
  ...
  describe('authenticate user', () => {
    ...
    it('should fail on authenticate with incorrect email', async () => {
      ...
      try {
        const auth = await projectZApi.authenticateUser(email, password)
      } catch (error) {
        errorCatched = error
      }
    ...
    it('should fail on authenticate with incorrect username', async () => {
      ...
      try {
        const auth = await projectZApi.authenticateUser(username, password)
      } catch (error) {
        errorCatched = error
      }
  ...
  describe('retrieve user by username', () => {
    ...
    it('should fail on incorrect username', async () => {
      ...
      try {
        const auth = await projectZApi.retrieveUserInfoByUsername(username)
      } catch (error) {
        errorCatched = error
      }

      expect(errorCatched instanceof Error).toBe(true)
      expect(errorCatched.message).toBe(`user with id ${username} not found`)
    })
  ...
  describe('retrieve game info', () => {
    ...
    it('should fail when no games found', async () => {
      ...
      expect(errorCatched instanceof Error).toBe(true)
      expect(errorCatched.message).toBe(`${gameId} doesn't exist in database`)
    })
  ...
  describe('Post Review', () => {
    ...
    beforeEach(async () => {
      ...
      const {token} = await projectZApi.authenticateUser(email, password)
      _token = token
    })
    ...
    it('should fail when same user tries to review twice a game', async () => {
      ...
      expect(
        async () =>
          await projectZApi
            .postReview(_token, gameId, text, score)
            .toThrow(new DuplicateError('user reviewed this game before'))
      )
    ...
    it('should fail on empty token', () => {
      ...
      expect(() => projectZApi.postReview(_token, gameId, text, score)).toThrow(
        new EmptyError('token is empty or blank')
      )
    })
    ...
    it('should fail on non integer score review', () => {
      ...
      expect(() => projectZApi.postReview(_token, gameId, text, score)).toThrow(
        Error('score should be an integer number')
      )
    })
  })
