...
const Review = ({index, history, review, printFrom}) => {
  ...
  const starScores = score => {
    ...
    for (let i = 0; i < score; i++) {
      stars.push(<i key={i} className="game-profile-stars fas fa-star" />)
    }
    return stars
  }

  const reviewClass =
    index % 2 === 0 ? 'review-card review-card--background-even' : 'review-card'

  return (
    <article className={reviewClass}>
      <div>
        {review.author !== null ? (
          printFrom === 'gameProfile' || printFrom === 'landingPage' ? (
            <div>
              <img
                onClick={() => history.push(`/${review.author.username}`)}
      ...
      <div>
        {(printFrom === 'userProfile' || printFrom === 'landingPage') && (
          <h3>
            <Link to={`/game/${review.game.id}`}>{review.game.game_title}</Link>
          </h3>
        )}
        ...
        {review.author !== null ? (
          printFrom === 'gameProfile' ? (
            ...
            <h3>
              <Link to={`/${review.author.username}`}>
                <span className="review-landing">{review.author.username}</span>
              </Link>
            </h3>
          ) : (
            ''
