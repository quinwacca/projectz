...
const RecommendedReviews = () => {
  ...
  const getSimilarUsersReviews = async () => {
    ...
    if (user.reviews.length === 0) {
      setNewbieMessage(
        <p>
          Start rating games to receive personalized reviews from similar users!
        </p>
      )
      return false
    }
    const {similarityList, allUsers} = await logic.retrieveSimilarUsersReviews()
      ...
      for (let i = 0; i < 5; i++) {
        _filterFive.push(
          _filterRecommendations.splice(
            Math.floor(Math.random() * _filterRecommendations.length - 1),
            1
          )[0]
        )
      }
        ...
        <div className="top-landing__results">
          {recommendations.length ? (
            ...
            <div className="no-recommendations">
              ...
              <p>
                {' '}
                Keep calm and <Link to="/random">RATE GAMES</Link>!
              </p>
            </div>
          )}
