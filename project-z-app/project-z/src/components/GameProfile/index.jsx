...
const GameProfile = ({
  ...
  useEffect(
    ...
    [gameInfo, getPreScore]
  )

  const retrieveGameInfo = useCallback(async gameId => {
    try {
      logic
        .retrieveGameInfo(gameId)
          ...
          <Fragment>
            {logic.isUserLoggedIn &&
              ...
              (gameInfo.reviews.some(
                review =>
                  review.author !== null && review.author.username === username
              ) ? (
                ''
              ) : (
            ...
            <div className="game-profile">
              {gameInfo.boxartUrl ? (
                ...
                <img src={NOT_FOUND_IMAGE_URL} alt={gameInfo.game_title} />
              )}
              ...
              <p className="game-profile__overview">{gameInfo.overview}</p>
            </div>
            <div>
              {logic.isUserLoggedIn ? (
                gameInfo.reviews.some(
                  ...
                  <div>
                    <h2 className="header header__title">Write Review</h2>
                    <WriteReview
                      gameInfo={gameInfo}
            ...
            <div>
              {gameInfo.reviews &&
                !!gameInfo.reviews.length && (
                  <div>
                    <h2 className="header header__title">REVIEWS</h2>
